...
class TokenlessAzureHandler(BaseTokenlessUploadHandler):
    ...
    def verify(self):
        ...
        if not self.upload_params.get('server_uri'): raise NotFound('Missing "server_uri" argument. Please upload with the Codecov repository upload token to resolve issue.')
        ...
        if(build['status'] == 'completed'):
            ...
            buildFinishDateObj = datetime.strptime(finishTimestamp, '%Y-%m-%d %H:%M:%S.%f')
        ...
        if build['buildNumber'] != self.upload_params.get('build'):
            ...
            raise NotFound('Build numbers do not match. Please upload with the Codecov repository upload token to resolve issue.')
