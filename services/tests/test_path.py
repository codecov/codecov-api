...
class TestPath(TestCase):
    ...
    def test_path_tree_with_many_paths(self):
        ...
        commit_report = SerializableReport(files=files)
        paths = [FilteredFilePath(full_path=path, stripped_path=path) for path in files.keys()]
        tree = path_tree(paths, commit_report)
    ...
    def test_filter_files_by_empty_path_prefix(self):
        file_list = ['fileA.py', 'folder/fileB.py', 'folder/subfolder/fileC.py', 'folder/subfolder/fileD.py']
        path_prefix = ''

        filtered_file_paths = filter_files_by_path_prefix(file_list, path_prefix)

        expected_result = [
            FilteredFilePath(full_path='fileA.py',
            stripped_path='fileA.py'),
            FilteredFilePath(full_path='folder/fileB.py',
            stripped_path='folder/fileB.py'),
            FilteredFilePath(full_path='folder/subfolder/fileC.py',
            stripped_path='folder/subfolder/fileC.py'),
            FilteredFilePath(full_path='folder/subfolder/fileD.py',
    ...
    def test_filter_files_by_empty_path_prefix(self):
        ...
        expected_result = [
            FilteredFilePath(full_path='folder/subfolder/fileC.py',
            stripped_path='fileC.py'),
            FilteredFilePath(full_path='folder/subfolder/fileD.py',
            stripped_path='fileD.py')
        ]
