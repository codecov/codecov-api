...
class OwnerManagerTests(TestCase):
    ...
    def test_annotate_with_latest_pr_date_in(self):
        ...
        with self.subTest("with pulls in separate orgs"):
            ...
            most_recent_pull_in_org = G(Pull, repository=repository_in_org, author=self.owner)
            more_recent_pull_not_in_org = G(Pull, repository=repository_not_in_org, author=self.owner)
    ...
    def test_annotate_with_lastseen(self):
        ...
        owner = Owner.objects.annotate_with_lastseen(
        ).get(
            ownerid=self.owner.ownerid
        )
