...
class SetYamlOnOwnerInteractor(BaseInteractor):
    ...
    def yaml_side_effects(self, old_yaml: dict, new_yaml: dict):
        old_yaml_branch = old_yaml.get("codecov", {}).get("branch")
        new_yaml_branch = new_yaml.get("codecov", {}).get("branch")

        # Update all repositories from owner if branch is updated in yaml
        if new_yaml_branch != old_yaml_branch:
            repos = Repository.objects.filter(self.owner.ownerid)
            repos.update(
                branch=new_yaml_branch or old_yaml_branch
            )  # Keeps old_branch if new_branch is None

        old_yaml_bot = old_yaml.get("codecov", {}).get("bot")
        new_yaml_bot = new_yaml.get("codecov", {}).get("bot")
