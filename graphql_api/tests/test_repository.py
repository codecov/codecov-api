...
class TestFetchRepository(GraphQLTestHelper, TransactionTestCase):
    ...
    #
    @override_settings(IS_ENTERPRISE=True, GUEST_ACCESS=False)
    def test_fetch_public_repo_when_guest_access_off(self):
        ...
        # confirm cannot see public repo
        try:
            data = self.gql_request(
                query_not_my_repo,
                owner=user,
        ...
        # confirm cannot see private repo
        try:
            data = self.gql_request(
                query_not_my_repo,
                owner=user,
                variables={"name": private_repo.name, "owner": owner.username},
            )
            print(data)
        except GraphQLError as e:
            assert e.message == UnauthorizedGuestAccess.message
    ...
    def test_fetch_public_repo_when_guest_access_on(self):
        user = OwnerFactory(username="sample-user")
        owner = OwnerFactory(username="sample-owner", plan_activated_users=[123, 456])
        user.save()
        owner.save()
        ...
        assert data["owner"]["repository"]["name"] == public_repo.name
        ...
        assert data["owner"]["repository"]["__typename"] == "NotFoundError"
