...
class CompareViewSet(CompareSlugMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    def get_object(self):
        ...
        if "pull" in compare_data:
            try:
                comparison = PullRequestComparison(user=self.request.user, pull=compare_data["pull"])
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if isinstance(comparison, PullRequestComparison):
            if comparison.pseudo_diff_adjusts_tracked_lines and comparison.allow_coverage_offsets:
                comparison.update_base_report_with_pseudo_diff()
            elif comparison.pseudo_diff_adjusts_tracked_lines:
                return Response(
                    data={"detail":
                        f"Changes found in between %.7s...%.7s (pseudo...base) "
