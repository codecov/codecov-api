...
class SettingsViewsetCloudTestCase(TestCase):
    def setUp(self):
    ...
    def test_settings(self):
        res = self.client.get("/internal_api/self_hosted/settings/")
        assert res.status_code == 404

    def test_settings_update(self):
        res = self.client.patch(
            "/internal_api/self_hosted/settings/",
            data={"autoactivation_enabled": True},
            format="json",
        )
        assert res.status_code == 404


@override_settings(IS_ENTERPRISE=True, ROOT_URLCONF="internal_api.enterprise_urls")
