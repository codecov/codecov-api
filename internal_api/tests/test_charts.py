...
class TestChartQueryRunnerQuery(TestCase):
    ...
    def test_query_aggregates_multiple_repository_totals(self):
        query_runner = ChartQueryRunner(
            ...
            request_params={
                ...
                "end_date": str(datetime.now(tz=UTC)),
                "grouping_unit": "day"
            }
        )
    ...
    def test_query_aggregates_with_latest_commit_if_no_recent_upload(self):
        ...
        self.commit1.timestamp = datetime.now(tz=UTC) - timedelta(days=7)
        self.commit1.save()
        ...
        query_runner = ChartQueryRunner(
            ...
            request_params={
                "owner_username": self.org.username,
                "service": self.org.service,
                "start_date": str(datetime.now(tz=UTC) - timedelta(days=1)),
                "grouping_unit": "day"
            }
        )
...
class TestChartQueryRunnerHelperMethods(TestCase):
    ...
    def test_first_complete_commit_date_returns_date_of_first_complete_commit_in_repoids(self):
        ...
        older_incomplete_commit = G(
            ...
            branch=repo1.branch,
            state="pending",
            timestamp=datetime.now(tz=UTC) - timedelta(days=7)
        )
        commit1 = G(
            model=Commit,
            repository=repo1,
    ...
    def test_end_date(self):
        ...
        with self.subTest("returns datetime.now(tz=UTC) if not supplied"):
            assert ChartQueryRunner(
                self.user,
                {
                    "owner_username": self.org.username,
            ...
            ).end_date == datetime.date(datetime.now(tz=UTC))
