...
class TestFlagSerializers(object):
    def test_flag_serializer(self, mocker, db, codecov_vcr):
        ...
        head_f = open(
            current_file.parent / f"samples/{head_commit_sha}_chunks.txt", "r"
        )
        base_f = open(
            current_file.parent / f"samples/{base_commit_sha}_chunks.txt", "r"
        )
        mocker.patch.object(ArchiveService, "create_root_storage")
        mocked.side_effect = (
            lambda x: head_f.read() if x == head_commit_sha else base_f.read()
        )
        ...
        base_commit = CommitFactory.create(
            ...
            report=json.load(
                open(current_file.parent / f"samples/{base_commit_sha}_report.json")
            ),
        )
        commit = CommitFactory.create(
            ...
            totals={
                ...
                "p": 0,
                "s": 2,
                "diff": [1, 0, 0, 0, 0, None, 0, 0, 0, 0, None, None, 0],
            },
            report=json.load(
                open(current_file.parent / f"samples/{head_commit_sha}_report.json")
            ),
        )
        comparison = Comparison(base_commit, commit, user=repo.author)
        flag_comparison = comparison.flag_comparison("flagtwo")
        res = FlagComparisonSerializer(
            instance=flag_comparison, context={"user": repo.author}
        ).data
        expected_result = {
            "name": "flagtwo",
            "base_report_totals": {
        ...
        mocked.assert_called_with(head_commit_sha)
        mocked.assert_any_call(base_commit_sha)
        assert expected_result == res
        second_flag_comparison = comparison.flag_comparison("flagone")
        second_res = FlagComparisonSerializer(
            instance=second_flag_comparison, context={"user": repo.author}
        ).data
        ...
        assert (
            second_res["base_report_totals"]
            == second_expected_result["base_report_totals"]
        )
        assert second_res["diff_totals"] == second_expected_result["diff_totals"]
        assert (
            second_res["head_report_totals"]
            == second_expected_result["head_report_totals"]
        )
