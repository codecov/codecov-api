...
def tree(parsed_data, href=None, classes=None, **kwargs):
    ...
    def recursively_draw(items, step, left, top, width, height):
        ...
        if _sum_values > 0:
            ...
            sorted_values = sorted(enumerate(value * correction for value in values), key=itemgetter(1), reverse=True)
            ...
            for rect, color, _class, children, name in zip(
                ...
                if children:
                    ...
                    path = '/'.join(step[1:])
                    rect = _svg_rect(rect[0], rect[1], rect[2], rect[3],
                                     fill=color,
    ...
    return _make_svg(options['width'], options['height'], svg_elements,
                     options.get('viewPortWidth'), options.get('viewPortHeight'))
    ...
    def recursively_draw(items, x, y, max_width, prefix_name):
        ...
        if total > 0:
            for item in items:
                ...
                svg_elements.append(_svg_rect(x, y, item_width, strip_height,
                                              fill=item['color'], stroke=options['border_color'],
                                              title=title,
                                              stroke_width=options['border_size']))
                if 'children' in item.keys():
                    recursively_draw(item['children'], x, y + strip_height,
                                     item_width,
                                     title)
                x += item_width
...
def sunburst(parsed_data, **kwargs):
    ...
    # ensure 5% frame
    max_diameter = min(drawing_width, drawing_height) * .95
    ...
    def recursively_draw(items, inner_radius, start, end):
        ...
        if total > 0:
            ...
            for item in items:
                ...
                svg_elements.append(
                    _svg_polar_rect(cx, cy, inner_radius, inner_radius + offset_increment, s, s + arc_size,
                                    item['color'],
                                    options['border_color'], options['border_size']))
                if 'children' in item.keys():
                    recursively_draw(item['children'], inner_radius + offset_increment, s, s + arc_size)
                s += arc_size
